...
### Requirement: WPA list tiles SHALL use citizen list card styling

Application and search result tiles on WPA officer views SHALL use the citizen list card pattern (`shadow-sm`, `border-none`, icon tile, optional chevron) rather than inset `bg-muted/30` rows inside a parent Card.

#### Scenario: Application tile interaction

- **WHEN** officer views a pending application on the dashboard or applications list
- **THEN** system SHALL render each item as an individual list card tile
- **AND** tapping the tile (chevron affordance) SHALL navigate to application details
- **AND** system SHALL NOT require a separate Rozpatrz or Szczegóły button on the list row

#### Scenario: Tab counts beside labels

- **WHEN** officer views WPA dashboard tabs with pending counts
- **THEN** system SHALL show the count in a badge beside the tab label
- **AND** system SHALL NOT use an absolute-positioned notification dot on the tab icon only

### Requirement: WPA list section behavior SHALL remain unchanged

UI alignment SHALL NOT alter application fetching, filtering, or alert suspend business logic.

#### Scenario: Alert actions preserved

- **WHEN** officer uses alert footer actions (profile, update exams, suspend permit)
